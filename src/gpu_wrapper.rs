...
impl<'a> GpuWrapper<'a> {
    ...
    fn create_compute_pipeline(device: &Device) -> wgpu::ComputePipeline {
        ...
        let bind_group_layout = device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
            label: None,
            entries: &[
                wgpu::BindGroupLayoutEntry { // The output texture
                    binding: 0,
                ...
                wgpu::BindGroupLayoutEntry { // The uniform buffer
                    binding: 1,
                    visibility: wgpu::ShaderStages::COMPUTE,
    ...
    fn create_render_pipeline(device: &Device, vertex_buffer_layout: wgpu::VertexBufferLayout) -> wgpu::RenderPipeline {
        ...
        let bind_group_layout = device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
            label: None,
            entries: &[
                wgpu::BindGroupLayoutEntry { // The input texture, copied from the compute's output texture
                    binding: 0,
                ...
                wgpu::BindGroupLayoutEntry { // The nearest-neighbor sampler
                    binding: 1,
                    visibility: wgpu::ShaderStages::FRAGMENT,
