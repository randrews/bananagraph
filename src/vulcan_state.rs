...
pub fn random_vulcan(mode: u8) -> [u8; 131072] {
    ...
    let palette_bytes: [u8; 48] = [
        ...
        0xff, 0x77, 0xa8,
        0xff, 0xcc, 0xaa,
        //0x00, 0x05, 0x65, 0x11, 0xa8, 0x49, 0xeb, 0xff, 0xe1, 0xf4, 0xfc, 0x1c, 0x37, 0x8e, 0xee, 0xfa,
    ];
    let palette_start = (0x20000 - 0x100) as usize;
    buf[palette_start..(palette_start + palette_bytes.len())].copy_from_slice(&palette_bytes);
...
mod test {
    ...
    fn test_random_vulcan() {
        ...
        assert_eq!(mem[0x20000 - 0x100 + 3], 0x1d); // Spot check the palette
    }
}